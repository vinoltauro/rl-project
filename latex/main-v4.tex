...
\centering
\begin{tikzpicture}[
    phase/.style={rectangle, draw=tcdBlue, fill=tcdBlue!10, rounded corners=5pt,
                  minimum width=3.2cm, minimum height=1.4cm, text centered,
                  font=\small, text width=3cm},
    finding/.style={rectangle, draw=gray!60, fill=gray!8, rounded corners=3pt,
                    minimum width=3.2cm, minimum height=0.7cm, text centered,
                    font=\footnotesize, text width=3cm},
    arr/.style={-Stealth, thick, tcdBlue},
    node distance=0.6cm and 0.7cm
]
\node[phase] (p1) {\textbf{Phase 1}\\Baseline 2$\times$2\\Representation Study};
\node[phase, right=of p1] (p2) {\textbf{Phase 2}\\Cross-Game\\Transfer};
\node[phase, right=of p2] (p3) {\textbf{Phase 3}\\Sequential\\Forgetting};
\node[phase, right=of p3] (p4) {\textbf{Phase 4}\\Interleaved\\Training};

\node[finding, below=0.3cm of p1] (f1) {CKA: conv general,\\fc\_repr specific};
\node[finding, below=0.3cm of p2] (f2) {Similarity $\neq$\\compatibility};
\node[finding, below=0.3cm of p3] (f3) {Forgetting via\\fc\_repr};
\node[finding, below=0.3cm of p4] (f4) {92\% dead neurons,\\both tasks fail};

\draw[arr] (p1) -- (p2);
\draw[arr] (p2) -- (p3);
\draw[arr] (p3) -- (p4);
\draw[dashed, gray] (p1) -- (f1);
\draw[dashed, gray] (p2) -- (f2);
\draw[dashed, gray] (p3) -- (f3);
\draw[dashed, gray] (p4) -- (f4);
\end{tikzpicture}
\caption{Four-phase experimental design. Each phase is motivated by the finding of the
previous one: CKA establishes which layers are game-specific, transfer experiments test
whether similarity implies reusability, sequential training confirms where forgetting
propagates, and interleaved training tests whether joint training can avoid it.}
\label{fig:overview}
\end{figure}
...
\begin{tikzpicture}[
    ...
    inputbox/.style={box, fill=gray!15, minimum width=2cm},
    convbox/.style={box, fill=tcdBlue!20, minimum width=2.6cm},
    fcbox/.style={box, fill=orange!25, minimum width=2.6cm},
    reprbox/.style={box, fill=green!25, minimum width=2.6cm, very thick},
    outbox/.style={box, fill=red!20, minimum width=2.6cm},
    arr/.style={-Stealth, thick},
    node distance=0.45cm
]
\node[inputbox] (input) {Input\\(4, 84, 84)};
\node[convbox, right=of input] (conv1) {Conv1\\32$\times$8$\times$8, s4\\ReLU};
\node[convbox, right=of conv1] (conv2) {Conv2\\64$\times$4$\times$4, s2\\ReLU};
\node[convbox, right=of conv2] (conv3) {Conv3\\64$\times$3$\times$3, s1\\ReLU};
\node[fcbox, right=of conv3] (flat) {Flatten\\3,136};
\node[reprbox, right=of flat] (repr) {\textbf{fc\_repr}\\512-dim\\ReLU};
\node[outbox, right=of repr] (out) {fc\_out\\$n$ actions\\Q-values};
...
% Hook annotation
\draw[dashed, thick, tcdBlue] (repr.north) -- ++(0,0.55)
    node[above, font=\footnotesize\color{tcdBlue}] {forward hook (all analysis here)};

% Parameter counts below
\node[font=\tiny, below=0.15cm of conv1] {$\approx$21k params};
\node[font=\tiny, below=0.15cm of conv2] {$\approx$164k params};
\node[font=\tiny, below=0.15cm of conv3] {$\approx$37k params};
\node[font=\tiny, below=0.15cm of repr]  {$\approx$1.6M params};
\node[font=\tiny, below=0.15cm of out]   {$\approx$2k params};
\end{tikzpicture}
\caption{CNN architecture shared by all agents. The \texttt{fc\_repr} layer (green) is the
focus of all representation analysis: a forward hook captures its 512-dimensional output
after every forward pass. The \texttt{fc\_out} head is game-specific (Pong: 6 actions,
Breakout: 4 actions). Approximately 95\% of the $\sim$1.7M total parameters reside in
\texttt{fc\_repr}.}
