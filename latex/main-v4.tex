...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[
    phase/.style={rectangle, draw=tcdBlue, fill=tcdBlue!12, rounded corners=6pt,
                  minimum width=3.4cm, minimum height=1.5cm, text centered,
                  font=\small\bfseries, text width=3.2cm, line width=1.2pt},
    finding/.style={rectangle, draw=gray!50, fill=gray!6, rounded corners=4pt,
                    minimum width=3.4cm, minimum height=0.8cm, text centered,
                    font=\footnotesize, text width=3.2cm},
    arr/.style={-Stealth, line width=1.2pt, tcdBlue},
    darr/.style={dashed, gray!60, line width=0.7pt},
    node distance=0.8cm and 0.9cm
]
\node[phase] (p1) {Phase 1\\[3pt]\normalfont Baseline 2$\times$2\\Representation Study};
\node[phase, right=of p1] (p2) {Phase 2\\[3pt]\normalfont Cross-Game\\Transfer};
\node[phase, right=of p2] (p3) {Phase 3\\[3pt]\normalfont Sequential\\Forgetting};
\node[phase, right=of p3] (p4) {Phase 4\\[3pt]\normalfont Interleaved\\Training};

\node[finding, below=0.45cm of p1] (f1) {CKA: conv general,\\fc\_repr game-specific};
\node[finding, below=0.45cm of p2] (f2) {High CKA $\neq$\\functional compatibility};
\node[finding, below=0.45cm of p3] (f3) {Forgetting localised\\to fc\_repr};
\node[finding, below=0.45cm of p4] (f4) {92\% dead neurons,\\neither task learned};

\draw[arr] (p1) -- (p2) node[midway, above, font=\scriptsize\color{tcdBlue}]{motivates};
\draw[arr] (p2) -- (p3) node[midway, above, font=\scriptsize\color{tcdBlue}]{motivates};
\draw[arr] (p3) -- (p4) node[midway, above, font=\scriptsize\color{tcdBlue}]{motivates};
\draw[darr] (p1) -- (f1);
\draw[darr] (p2) -- (f2);
\draw[darr] (p3) -- (f3);
\draw[darr] (p4) -- (f4);
...
\begin{tikzpicture}[
    node distance=0.5cm,
    layer/.style={
        rectangle, draw=gray!60, fill=white, rounded corners=4pt,
        minimum width=2.0cm, minimum height=1.6cm,
        text centered, font=\small, line width=0.6pt
    },
    convlayer/.style={layer, fill=tcdBlue!12, draw=tcdBlue!60},
    fclayer/.style={layer, fill=orange!12, draw=orange!60},
    reprlayer/.style={layer, fill=green!15, draw=green!60, line width=1.4pt},
    outlayer/.style={layer, fill=red!10, draw=red!50},
    inputlayer/.style={layer, fill=gray!10, draw=gray!50},
    arr/.style={-Stealth, line width=0.9pt, gray!70},
    label/.style={font=\scriptsize\color{gray!80}, below=0.08cm}
]

%% Nodes
\node[inputlayer] (input) {%
    \textbf{Input}\\[2pt]
    \scriptsize 4$\times$84$\times$84};

\node[convlayer, right=of input] (conv1) {%
    \textbf{Conv1}\\[2pt]
    \scriptsize 32, 8$\times$8, s4\\
    \scriptsize ReLU};

\node[convlayer, right=of conv1] (conv2) {%
    \textbf{Conv2}\\[2pt]
    \scriptsize 64, 4$\times$4, s2\\
    \scriptsize ReLU};

\node[convlayer, right=of conv2] (conv3) {%
    \textbf{Conv3}\\[2pt]
    \scriptsize 64, 3$\times$3, s1\\
    \scriptsize ReLU};

\node[fclayer, right=of conv3] (flat) {%
    \textbf{Flatten}\\[2pt]
    \scriptsize 3{,}136};

\node[reprlayer, right=of flat] (repr) {%
    \textbf{fc\_repr}\\[2pt]
    \scriptsize 512-dim\\
    \scriptsize ReLU};

\node[outlayer, right=of repr] (out) {%
    \textbf{fc\_out}\\[2pt]
    \scriptsize $n$ actions\\
    \scriptsize Q-values};

%% Arrows
\foreach \from/\to in {input/conv1, conv1/conv2, conv2/conv3, conv3/flat, flat/repr, repr/out}
    \draw[arr] (\from) -- (\to);

%% Output size annotations below nodes
\node[label] at (conv1.south) {32$\times$20$\times$20};
\node[label] at (conv2.south) {64$\times$9$\times$9};
\node[label] at (conv3.south) {64$\times$7$\times$7};

%% Forward hook annotation above fc_repr
\draw[dashed, tcdBlue!70, line width=0.8pt]
    (repr.north) -- ++(0, 0.5)
    node[above, font=\scriptsize\bfseries\color{tcdBlue}]
        {$\hookleftarrow$ forward hook};

%% Convolutional backbone brace
\begin{scope}[on background layer]
    \node[draw=tcdBlue!30, fill=tcdBlue!4, rounded corners=5pt,
          fit=(conv1)(conv3), inner sep=5pt] (backbone) {};
\end{scope}
\node[font=\scriptsize\color{tcdBlue!70}, above=0.05cm of backbone]
    {Convolutional backbone (shared, 1--3)};

\end{tikzpicture}
}% end resizebox
\caption{CNN architecture shared by DQN and DDQN across all experiments. Blue nodes form
the convolutional backbone; the green \texttt{fc\_repr} node is the 512-dimensional
representation layer where all analysis is performed. A PyTorch forward hook captures
activations here after every forward pass. The \texttt{fc\_out} head is game-specific
(Pong: 6 actions; Breakout: 4 actions). Approximately 95\% of the total $\sim$1.7M
parameters reside in \texttt{fc\_repr}. Output feature map sizes are shown below each
convolutional layer.}
...
are measured at fc\_repr. Breakout performance for sequential conditions is estimated from
the no-freeze run; freeze-all Breakout is not optimised (only the 2,052-parameter output
head trains).}
\label{tab:master}
\begin{tabularx}{\linewidth}{lXXXX}
\toprule
\textbf{Condition} & \textbf{Pong (final)} & \textbf{Breakout (final)} & \textbf{Dead neurons} & \textbf{Key finding} \\
\midrule
DQN standalone       & $7.12 \pm 5.22$  & $3.86 \pm 4.94$ & baseline & Task-specific fc\_repr \\
DDQN standalone      & $7.50 \pm 5.03$  & $4.00 \pm 5.04$ & lower    & Tighter representations \\
\midrule
Scratch Breakout     & --               & 3.86            & --       & Baseline for transfer \\
Frozen conv (Pong)   & --               & 2.28            & --       & Conv too Pong-specific \\
Full fine-tune       & --               & 3.12 (peak 4.47)& --       & Best Breakout peak \\
\midrule
Sequential no freeze & $-$20.8          & $\sim$3.5       & 0.87     & Immediate forgetting \\
Sequential freeze conv & $-$14.0        & $\sim$2.5       & 0.84     & fc\_repr still forgets \\
