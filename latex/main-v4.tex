...
previously learned information when a neural network is trained on a new task. In deep RL,
the problem is compounded by non-stationarity: unlike supervised learning, where the data
...
Raw 210$\times$160 RGB frames are transformed through the standard DeepMind preprocessing
stack \cite{mnih2015human}: no-op reset, frame skip (4), episodic life, grayscale and
resize to 84$\times$84, reward clipping to $\{-1, 0, +1\}$, and frame stacking (4 frames).

Both agents share the CNN from Mnih et al.: three convolutional layers (32 filters,
8$\times$8, stride 4; 64 filters, 4$\times$4, stride 2; 64 filters, 3$\times$3, stride 1),
...
initialisation \cite{saxe2013exact}.
...
Both algorithms reach comparable final reward on both games. On Pong, DQN achieves
$7.12 \pm 5.22$ and DDQN $7.50 \pm 5.03$. On Breakout, DQN reaches $3.86 \pm 4.94$ and
DDQN $4.00 \pm 5.04$. The near-identical task performance is important: any representational
differences observed in subsequent analysis cannot be attributed to differences in what the
agents learned to do, only to differences in how they represent it internally.
...
2.05, approximately 29\% lower. On Breakout the gap is smaller, with DQN showing a higher
...
The game played is the primary organising principle of the representation space; the
algorithm used is secondary.
...
Within each game, DDQN produces noticeably tighter cluster geometry with lower intra-cluster
variance (Figure~\ref{fig:algo_effect}). This is consistent with DDQN's more stable gradient
signal producing more organised state groupings in the representation space.
...
characterised by Sokar et al.\ \cite{sokar2023dormant}. Cross-game cosine similarity is
non-trivial for both algorithms, with DDQN showing marginally higher alignment
(Figure~\ref{fig:cosine}), suggesting its representations preserve more of the shared
visual content that Pong and Breakout genuinely have in common.
...
Under DDQN, cross-game CKA is 0.92 at conv1, 0.85 at conv2, 0.41 at conv3, and just 0.09
at fc\_repr, a tenfold drop from early to late layers. The pattern holds under DQN (0.68,
...
The native agent scores $3.40 \pm 3.82$. On Pong, any foreign conv layer, whether from
...
Frozen Pong conv underperforms scratch throughout training, despite CKA showing those layers
...
The interleaved results are unambiguous: step-level alternation with equal gradient signal
from both games produces complete failure on both tasks. Pong reward reaches $-21$ within
...
The theoretical framework of Elhage et al.\ \cite{elhage2022superposition} offers a precise
explanation. When a neural network with limited representational capacity is required to
encode two distinct, dense feature sets, it is forced past a phase change threshold into
superposition: features from both tasks compete for the same dimensions in fc\_repr. Because
Pong and Breakout features are not mutually sparse, the interference this creates within the
512-dimensional space is irresolvable. The network responds by saturating its ReLU units to
silence the interference, driving the dead neuron fraction to 92\%. The network is not
failing to learn because it lacks enough training steps; it is actively pruning itself to
eliminate the noise created by conflicting gradient objectives.
...
\textbf{Single random seed.} All experiments use seed 42. Running multiple seeds would be
needed to confirm the observed patterns are robust rather than initialisation-specific, and
is the most significant methodological limitation of this study.
