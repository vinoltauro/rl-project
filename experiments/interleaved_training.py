"""
Interleaved Training Experiment
=================================
A single DQN agent with a shared backbone and two output heads alternates
between Pong and Breakout episode by episode.
...
Training:
    Pong episode → Breakout episode → Pong episode → ...
...
Metrics logged every 100k total steps:
    ...
    3. Dead neurons     (fc_repr activation on 1000 shared Pong frames)
    4. Cross-game CKA   (similarity of Pong vs Breakout fc_repr activations)
                        High = shared backbone is general
                        Low  = backbone has specialised to one game

Key comparisons:
    - Pong reward: interleaved vs sequential (does interleaving prevent forgetting?)
    - Breakout reward: interleaved vs scratch (what is the interference cost?)
    - Dead neurons: interleaved vs sequential (does interleaving keep capacity alive?)
    - CKA: do representations converge or diverge under interleaving?
...
class InterleavedDQNAgent:
    """
    DQN agent with a shared AtariCNNTwoHead backbone.
    Routes action selection and learning through the correct game head.
    """
...
def measure_cross_game_cka(model, device):
    pong_reps     = collect_repr(model, "ALE/Pong-v5",     CFG["cka_states"],
                                  device, game="pong",     seed=42)
    breakout_reps = collect_repr(model, "ALE/Breakout-v5", CFG["cka_states"],
                                  device, game="breakout", seed=42)
    return linear_cka(pong_reps, breakout_reps)


# ── Main ──────────────────────────────────────────────────────────────────────
def train(steps_per_game: int):
    run_name = "dqn_interleaved_seed42_scalemedium"
    set_seeds(CFG["seed"])
    device = get_device()

    print(f"\n{'='*60}")
    print(f"  Interleaved Training Experiment")
    ...
    # One replay buffer and logger per game
    buffers = {
    ...
    mw.writerow(["total_step", "pong_steps", "breakout_steps",
                 ...
                 "dead_neurons", "cross_game_cka"])
    mf.flush()

    # Per-game step counters and episode reward trackers
    ...
    # Build environments (created fresh each episode to avoid state leakage)
    envs = {g: make_atari_env(GAMES[g][0], seed=CFG["seed"]) for g in GAMES}
    obs  = {g: envs[g].reset()[0] for g in GAMES}

    print(f"\n[training] Starting interleaved training...\n")

    # Stop when both games reach steps_per_game
    while any(game_steps[g] < steps_per_game for g in GAMES):
        game = game_order[current_game_idx % 2]
        current_game_idx += 1

        if game_steps[game] >= steps_per_game:
            continue

        # Play one full episode for this game
        ep_reward = 0
        ep_length = 0
        done      = False
        ...
        while not done:
            ...
            next_obs, reward, terminated, truncated, _ = envs[game].step(action)
            done = terminated or truncated

            buffers[game].push(obs[game], action, float(reward), next_obs, done)
            obs[game]  = next_obs
            ep_reward += reward
            ep_length += 1
            game_steps[game] += 1
            ...
            if (game_steps[game] >= CFG["learning_starts"]
                    and len(buffers[game]) >= CFG["batch_size"]):
    ...
    print(f"  Pong steps: {game_steps['pong']:,} | "
          f"Breakout steps: {game_steps['breakout']:,}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--steps_per_game", type=int, default=CFG["steps_per_game"])
